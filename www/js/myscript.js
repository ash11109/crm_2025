...
function print_shop_timeline(shop_data) {
    console.log(shop_data);

    const timeline_data = {
        item: [
            { shop_status: "Started" },
            { shop_status: "OTP verified" },
            { shop_status: "Shop Detail" },
            { shop_status: "Category" },
            { shop_status: "Shop" }
        ]
    };
    ...
    timeline_data.item.forEach((item, index) => {
        dataHolder += `
            <div class="time time${index + 1}" id="time${index + 1}">
                <h2>${item.shop_status}</h2>
                <p></p>
            </div>
        `;
    });

    document.getElementById("single_shop_data").innerHTML += `
        <div class="row">
            ${dataHolder}
        </div>
    `;

    for (let i = 0; i < shop_data.setup_stage ; i++) {
        const element = document.getElementById(`time${i + 1}`);
        if (element) {
    ...
    $("#single_shop_data").html(`
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    <th>Shop Name</th>
                    <td><a href="${data.shop_link}" target="_blank">website link</a></td>
                </tr>
                <tr>
                    <th>Email</th>
                    <td>${data.email}</td>
        ...
        <table class="table">
            ...
            <tbody>
                <tr>
                    ...
                    <td>${res.email_status == true ? 'verified' : 'not verified' }</td>
    ...
    function loadtable() {
      ...
      var table = $("#table1").DataTable({
        ...
        ajax: {
          url: Config.api_url,
          method: "POST",
          data: { operation: "20250001", option: option , srch : srch },
          beforeSend: function () {
            $(".loader").show();
          },
          complete: function () {
